...
export class SymbolicExpression {
  ...
  static parse(str: string): SymbolicExpression {
    ...
    const explicitMultMatch = str.match(
      /^(-?)(\d*\.?\d*)\s*\*\s*([a-zA-Z]\w*)$/,
    );
    ...
    for (const part of parts) {
      if (part === "+") {
        ...
        if (/^\d*\.?\d+$/.test(trimmed)) {
          terms.push({
            coefficient: currentSign * parseFloat(trimmed),
            variables: {},
          });
  ...
  isZero(): boolean {
    return (
      this.terms.length === 0 ||
      this.terms.every((t) => Math.abs(t.coefficient) < 1e-10)
    );
  }
  ...
  toLatex(): string {
    ...
    for (let i = 0; i < this.terms.length; i++) {
      ...
      if (vars === "") {
        ...
        const coefStr = Number.isInteger(absCoef)
          ? absCoef.toString()
          : absCoef.toFixed(2);
...
export function symbolicSubtract(
  a: SymbolicValue,
  b: SymbolicValue,
): SymbolicExpression {
  const aExpr = typeof a === "number" ? SymbolicExpression.fromNumber(a) : a;
...
export function symbolicMultiply(
  a: SymbolicValue,
  b: SymbolicValue,
): SymbolicExpression {
  const aExpr = typeof a === "number" ? SymbolicExpression.fromNumber(a) : a;
...
export function symbolicDivide(
  a: SymbolicValue,
  b: SymbolicValue,
): SymbolicExpression {
  const aExpr = typeof a === "number" ? SymbolicExpression.fromNumber(a) : a;
