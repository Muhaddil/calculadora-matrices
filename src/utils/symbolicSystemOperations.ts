...
export function solveSymbolicLinearSystem(
  ...
  try {
    ...
    steps.push({
      ...
      description: t("symbolicSystemOperations.originalSystemDescription", {
        n,
        m,
      ...
      matrices: [
        {
          ...
          customLabels: symbolicA.map((row) =>
            row.map((cell) => cell.toLatex()),
          ),
        },
        {
          label: "B",
          matrix: symbolicB.map((row) => row.map(() => 0)),
          customLabels: symbolicB.map((row) =>
            row.map((cell) => cell.toLatex()),
          ),
        },
      ],
    ...
    if (hasParameters) {
      steps.push({
        ...
        description: t(
          "symbolicSystemOperations.parametricDetectedDescription",
        ),
      });

      const result = analyzeParametricSystem(
        symbolicA,
        symbolicB,
        method,
        steps,
    ...
    return {
      solution: null,
      compatibility: t("symbolicSystemOperations.errorInCalc", {
        message:
          error instanceof Error
            ? error.message
            : t("symbolicSystemOperations.errorUnknown"),
      }),
      steps: [
        ...steps,
        {
...
function analyzeParametricSystem(
  ...
  if (n === m && n <= 3) {
    ...
    if (criticalValues.length > 0) {
      ...
      generalResult.compatibility = t(
        "symbolicSystemOperations.compatibleDeterminedGeneral",
      );
      ...
      for (const critical of criticalValues) {
        const caseSteps: CalculationStep[] = [
          {
            stepNumber: 1,
            title: t("symbolicSystemOperations.caseSpecialTitle", {
              condition: critical.condition,
            }),
            description: t("symbolicSystemOperations.caseSpecialDescription", {
              condition: critical.condition,
            }),
            formula: t("symbolicSystemOperations.substitutingFormula", {
              condition: critical.condition,
            }),
          },
        ];
        ...
        try {
          const substitutedA = substituteParameterSimple(
            A,
            critical.parameter,
            critical.value,
          );
          const substitutedB = substituteParameterSimple(
            B,
            critical.parameter,
            critical.value,
          );

          const substitutedEquations = substitutedA
            .map((row, i) => {
              const terms = row
                .map((coef, j) => {
                  if (coef.isZero()) return "";
          ...
          caseSteps.push({
            ...
            description: t(
              "symbolicSystemOperations.substitutedSystemDescription",
              { condition: critical.condition },
            ),
            formula: `\\begin{cases} ${substitutedEquations} \\end{cases}`,
          });

          const specialResult = solveByGauss(
            substitutedA,
            substitutedB,
            caseSteps,
          );
          ...
          caseSteps.push({
            ...
            description: t(
              "symbolicSystemOperations.substitutionErrorDescription",
              { condition: critical.condition },
            ),
          });
      ...
      if (specialCases.length > 0) {
        steps.push({
          ...
          description: t(
            "symbolicSystemOperations.casesIdentifiedDescription",
            { count: specialCases.length },
          ),
          formula: specialCases
            .map(
              (sc, i) =>
                `\\text{${t("symbolicSystemOperations.caseNumber", { i: i + 1 })}: } ${sc.condition}`,
            )
            .join("\\\\"),
        });
      }
...
function solveCriticalValues(
  det: SymbolicExpression,
): Array<{ parameter: string; value: number; condition: string }> {
  const results: Array<{
    parameter: string;
    value: number;
    condition: string;
      ...
      for (const solution of solutions) {
        if (solution.isConstant()) {
          ...
          const exists = results.some(
            (r) => r.parameter === param && Math.abs(r.value - value) < 1e-10,
          );
          if (!exists) {
            results.push({
              parameter: param,
...
function substituteParameterSimple(
...
function solveByCramer(
  ...
  if (criticalValues.length > 0) {
    ...
    const generalResult = continueCramerSolution(
      A,
      B,
      detA,
      criticalValues,
  ...
  if (detA.isZero()) {
    steps.push({
      ...
      description: t(
        "symbolicSystemOperations.singularNotApplicableDescription",
      ),
...
function continueCramerSolution(
  ...
  for (let i = 0; i < n; i++) {
    ...
    steps.push({
      stepNumber: steps.length + 1,
      title: t("symbolicSystemOperations.calculatingXTitle", {
        i: i + 1,
        step: i + 1,
      }),
      description: t("symbolicSystemOperations.calculatingXDescription", {
        i: i + 1,
        col: i + 1,
      }),
      matrices: [
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!simplified.equals(solution[i][0])) {
      steps.push({
        ...
        description: t("symbolicSystemOperations.simplifyXiDescription", {
          i: i + 1,
        }),
  ...
  return {
    ...
    compatibility:
      criticalValues.length > 0
        ? t("symbolicSystemOperations.compatibleDeterminedCritical", {
            conditions: criticalValues
              .map((cv) => `${cv.parameter} ≠ ${cv.value}`)
              .join(" y "),
          })
...
function trySimplifyFractionBetter(
  ...
  try {
    ...
    if (
      commonFactor &&
      !(
        commonFactor.isConstant() &&
        Math.abs(commonFactor.toNumber() - 1) < 1e-10
      )
    ) {
      ...
      if (simplifiedNum && simplifiedDen) {
        if (
          simplifiedDen.isConstant() &&
          Math.abs(simplifiedDen.toNumber() - 1) < 1e-10
        ) {
...
function solveByGauss(
  ...
  for (let col = 0; col < m && currentRow < n; col++) {
    ...
    if (pivotRow === -1) {
      steps.push({
        ...
        title: t("symbolicSystemOperations.columnNoPivotTitle", {
          col: col + 1,
        }),
        description: t("symbolicSystemOperations.columnNoPivotDescription", {
          col: col + 1,
        }),
      });
      continue;
    ...
    for (let row = currentRow + 1; row < n; row++) {
      if (!augmented[row][col].isZero()) {
        for (let j = col + 1; j <= m; j++) {
          ...
          const term2 = symbolicMultiply(
            augmented[row][col],
            augmented[currentRow][j],
          );
  ...
  steps.push({
    ...
    matrices: [
      {
        label:
          "\\text{" + t("symbolicSystemOperations.echelonMatrixTitle") + "}",
        matrix: augmented.map((row) => row.map(() => 0)),
        customLabels: augmented.map((row) => row.map((cell) => cell.toLatex())),
      },
    ],
    ...
    if (!augmented[i][m].isZero()) {
      ...
      if (allZero) {
        steps.push({
          ...
          description: t("symbolicSystemOperations.incompatibleDescription", {
            value: augmented[i][m].toLatex(),
          }),
        });
        return {
          solution: null,
  ...
  if (rankA === m) {
    const solution: SymbolicMatrix = Array(m)
      .fill(0)
      .map(() => [SymbolicExpression.fromNumber(0)]);
    ...
    for (let i = rankA - 1; i >= 0; i--) {
      ...
      for (let j = col + 1; j < m; j++) {
        sum = symbolicAdd(
          sum,
          symbolicMultiply(augmented[i][j], solution[j][0]),
        );
      }
    ...
    const solutionText = solution
      .map((row, i) => `x_{${i + 1}} = ${row[0].toLatex()}`)
      .join("\\\\ ");
...
function solveIndeterminateSystem(
  ...
  steps.push({
    ...
    description: t("symbolicSystemOperations.indeterminateDescription", {
      count: freeVars.length,
    }),
    formula: t("symbolicSystemOperations.indeterminateFreeVars", {
      vars: freeVars.map((v) => `x_{${v + 1}}`).join(", "),
    }),
  });
  ...
  const particularSolution: SymbolicMatrix = Array(m)
    .fill(0)
    .map(() => [SymbolicExpression.fromNumber(0)]);
  ...
  for (let i = pivotColumns.length - 1; i >= 0; i--) {
    ...
    for (let j = col + 1; j < m; j++) {
      sum = symbolicAdd(
        sum,
        symbolicMultiply(augmented[i][j], particularSolution[j][0]),
      );
    }
  ...
  steps.push({
    ...
    formula: t("symbolicSystemOperations.particularFoundFormula", {
      solution: particularSolution
        .map((row, i) => `x_{${i + 1}} = ${row[0].toLatex()}`)
        .join("\\\\ "),
    }),
  });
  ...
  steps.push({
    ...
    description: t("symbolicSystemOperations.homogeneousBaseDescription", {
      count: freeVars.length,
    }),
  });
  ...
  for (const freeVar of freeVars) {
    const basisVector: SymbolicMatrix = Array(m)
      .fill(0)
      .map(() => [SymbolicExpression.fromNumber(0)]);

    for (const fv of freeVars) {
      basisVector[fv][0] = SymbolicExpression.fromNumber(
        fv === freeVar ? 1 : 0,
      );
    }
    ...
    for (let i = pivotColumns.length - 1; i >= 0; i--) {
      ...
      for (let j = col + 1; j < m; j++) {
        sum = symbolicAdd(
          sum,
          symbolicMultiply(augmented[i][j], basisVector[j][0]),
        );
      }

      const pivot = augmented[i][col];
      const homogeneousNumerator = symbolicMultiply(
        SymbolicExpression.fromNumber(-1),
        sum,
      );
    ...
    steps.push({
      ...
      description: t("symbolicSystemOperations.basisVectorDescription", {
        i: freeVar + 1,
      }),
      formula: t("symbolicSystemOperations.basisVectorFormula", {
        i: freeVar + 1,
        vector: basisVector.map((row) => row[0].toLatex()).join("\\\\ "),
      }),
    });
  }
