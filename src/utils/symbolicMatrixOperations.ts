...
export function parseSymbolicMatrix(
  input: (string | number)[][],
): SymbolicMatrix {
...
export function calculateSymbolicDeterminant(
  ...
  steps.push({
    ...
    description: t("matrixOperations.symbolicDeterminantDescription", {
      n,
    ...
    matrices: [
      {
        label:
          "\\text{" + t("matrixOperations.matrixWithParametersLabel") + "}",
  ...
  if (det.isPolynomial()) {
    ...
    steps.push({
      ...
      description: t("matrixOperations.polynomialExpansionDescription", {
        variable,
      }),
    ...
    if (roots.length > 0) {
      steps.push({
        ...
        description: t("matrixOperations.rootsCalculationDescription", {
          variable,
        }),
...
function getMethodDescription(method: string, n: number): string {
  if (n === 3) {
    return method === "sarrus"
      ? t("matrixOperations.methodSarrus")
      : t("matrixOperations.methodCofactors");
  } else if (n >= 4) {
...
function calculateSymbolicDeterminant3x3Cofactors(
  ...
  const c12 = symbolicSubtract(
    symbolicMultiply(d, i),
    symbolicMultiply(f, g),
  ).negate();
...
function calculateSymbolicDeterminantWithSteps(
  ...
  if (n === 2) {
    ...
    const result = symbolicSubtract(
      symbolicMultiply(a, d),
      symbolicMultiply(b, c),
    );
    steps.push({
      stepNumber: steps.length + 1,
...
function calculateSymbolicDeterminantCofactors(
  ...
  steps.push({
    ...
    description: t("matrixOperations.expansionStrategyDescription", {
      row: expansionRow + 1,
    }),
  ...
  for (let j = 0; j < n; j++) {
    ...
    const signText =
      sign === 1
        ? t("matrixOperations.positive")
        : t("matrixOperations.negative");
    ...
    steps.push({
      stepNumber: steps.length + 1,
      title: t("matrixOperations.minorCalculationTitle", {
        row: expansionRow + 1,
        col: j + 1,
      }),
    ...
    const minorDet = calculateSymbolicDeterminantWithSteps(
      minor,
      steps,
      minorLabel,
    );
